...
namespace Jaeger.SAT.Catalogos.Update.Importers.Cfdi40 {
    ...
    /// </summary>
    public class ClavesAduanas : AbstractInjector, IInjector {
        public ClavesAduanas(DataTable dataTable) : base(dataTable) {
            _SkipRows = 3;
        }
        ...
        protected override void Fill() {
            var mapper = new Helpers.Mapping.DataNamesMapper<ClaveAduana>();
            var resultado = mapper.Map(_DataTable).ToList();
            if (resultado != null) {
                if (resultado.Count() > 0) {
                    _Catalogo = new AduanasRepository {
