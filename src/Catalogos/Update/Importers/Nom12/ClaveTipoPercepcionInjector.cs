...
namespace Jaeger.SAT.Catalogos.Update.Importers.Nom12 {
    ...
    /// </summary>
    internal class ClaveTipoPercepcionInjector : AbstractInjector, IInjector {
        public ClaveTipoPercepcionInjector(DataTable dataTable) : base(dataTable) {
            this.SkipRows = 3;
        }
        ...
        protected override void CreateRepository() {
            var mapper = new Helpers.Mapping.DataNamesMapper<CveTipoPercepcion>();
            var resultado = mapper.Map(_DataTable).ToList();
            if (resultado != null) {
                if (resultado.Count() > 0) {
                    _Repository = new TipoPercepcionRepository(this.LastVersion) {
