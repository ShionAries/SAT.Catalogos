...
namespace Jaeger.SAT.Catalogos.Update.Importers.Nomina {
    ...
    /// </summary>
    internal class ClavesRiesgoPuesto : AbstractInjector, IInjector {
        public ClavesRiesgoPuesto(DataTable dataTable) : base(dataTable) {
            this._SkipRows = 3;
        }
        ...
        protected override void CreateRepository() {
            var mapper = new Helpers.Mapping.DataNamesMapper<ClaveRiesgoPuesto>();
            var resultado = mapper.Map(_DataTable).ToList();
            if (resultado != null) {
                if (resultado.Count() > 0) {
                    _Catalogo = new RiesgoPuestoRepository {
